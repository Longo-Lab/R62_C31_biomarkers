...
# Absolute or relative path to the CSV file containing feautures. First row should contain field names.
INPUT_FILE = "knn_input.csv"

OUTPUT_FILE = "svm_rfc9.txt"
...
printer = OutputPrinter(OUTPUT_FILE)
...
def train_and_predict(train, test, active_features = []):
    ...
    for i, row in enumerate(test_x):
        prediction = rfc.predict([row])[0]
        predictions.append(prediction)
        probabilities = rfc.predict_proba([row])[0]
        prob_str = ""
    ...
    printer.print ("Total prediction score (mean accuracy): %f" % rfc.score(test_x, test_y))
    printer.print (classification_report(test_y, predictions))


input_set = CsvInputFile(INPUT_FILE)
...
for num_features in range(2, len(train[0][0]) + 1):
    rfc = RandomForestClassifier(n_estimators=N_ESTIMATORS)
    svr_estimator = SVC(kernel='linear')
    selector = RFE(estimator = svr_estimator, n_features_to_select=num_features, step=1)
    selector = selector.fit(train[0], train[1])
